...
add_shortcode('tabs', 'forceful_toolkit_shortcode_tabs');
add_shortcode('tab', '__return_false');
function forceful_toolkit_shortcode_tabs($atts, $content = null) {
    extract(shortcode_atts(array(), $atts));

    $items  = forceful_toolkit_get_shortcode($content, true, array('tab'));
    $navs   = array();
    $panels = array();
    ...
    if ($items) {
        ...
        foreach ($items as $item) {
            if(isset( $item['atts']['title'] )){
                $title = $item['atts']['title'];
            }else{
                $title = '';
            }
            $item_id  = 'tab-' . wp_generate_password(4, false, false);
            $navs[]   = sprintf('<li class="%s"><a href="#%s">%s</a></li>', $active, $item_id, do_shortcode($title));
            $panels[] = sprintf('<div class="tab-content-3 %s" id="%s">%s</div>', $active, $item_id, do_shortcode($item['content']));
            $active   = '';
        }
    }

    $output = '<div class="list-container-3">';
    
    $output .= '<ul class="tabs-3 clearfix">';
    $output .= implode('', $navs);
    $output .= '</ul>';
    
    $output .= '<div class="tab-container-3">';
    $output .= implode('', $panels);
    $output .= '</div>';
    
    $output .= '</div>';

    return apply_filters('forceful_toolkit_shortcode_tabs', $output, $atts, $content);
}
...
function kopa_shortcode_contact_form($atts, $content = null) {
    ...
    $out .= '<label class="required" for="contact_name">' . __('Name <span>(required):</span>', 'forceful-toolkit') . '</label>';
    $out .= '<input class="valid" type="text" name="name" id="contact_name" value="">';
    $out .= '</p>';

    $out .= '<p class="input-block clearfix">';
    $out .= '<label class="required" for="contact_email">' . __('Email <span>(required):</span>', 'forceful-toolkit') . '</label>';
    $out .= '<input type="email" class="valid" name="email" id="contact_email" value="">';
    $out .= '</p>';

    $out .= '<p class="input-block clearfix">';
    $out .= '<label class="required" for="contact_url">' . esc_html__('Website:', 'forceful-toolkit') . '</label>';
    $out .= '<input type="url" class="valid" name="url" id="contact_url" value="">';
    $out .= '</p>';

    $out .= '<p class="textarea-block">';
    $out .= '<label class="required" for="contact_message">' . __('Message <span>(required):</span>', 'forceful-toolkit') . '</label>';
    $out .= '<textarea id="contact_message" name="message"></textarea>';
    $out .= '</p>';

    $out .= '<p class="comment-button clearfix">';
    $out .= '<input type="submit" id="submit-contact" value="' . esc_html__('Submit', 'forceful-toolkit') . '">';
    $out .= '</p>';
...
function kopa_shortcode_posts($atts, $content = null) {
    ...
    if ($posts->post_count > 0) {
        ...
        while ($posts->have_posts()) {
            ...
            if (comments_open()) {
                if ($num_comments == 0) {
                    $comments = esc_html__('0 Comments', 'forceful-toolkit');
                } elseif ($num_comments > 1) {
                    $comments = $num_comments . esc_html__(' Comments', 'forceful-toolkit');
                } else {
                    $comments = esc_html__('1 Comment', 'forceful-toolkit');
                }
               $out .=  $comments;
            } else {
                $out .=  esc_html__('Comments off', 'forceful-toolkit');
            }
            $out .= '</span>';
            $out .= '</div>';
            $out .= '</article>';
